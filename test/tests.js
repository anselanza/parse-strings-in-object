...
console.log("parser", typeof parser, parser);
...
describe("Parsing", () => {
  ...
  it("should convert arrays of string representations of numbers into array of numbers", () => {
    const before = { foo: "true", list: ["1", "2", "3"] };
    const result = parser(before);
    expect(result).to.deep.equal({ foo: true, list: [1, 2, 3] });
    expect(result.list.length).to.equal(3);
  });

  it("should retain arrays of strings properly", () => {
